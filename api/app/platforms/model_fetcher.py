...
async def _fetch_openai(api_key: str) -> list[str]:
    ...
    # `-pro` is kept. OpenAI serves those models on the Responses API only, and
    # the monitoring adapter routes them there regardless of the web-grounding
    # setting (platforms/openai.OpenAIAdapter._call_api), so they are callable.
    # The analysis and recommendation engines still use v1/chat/completions and
    # would 404 on one, which model_registry.validate_model_config rejects.
