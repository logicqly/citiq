"""
One-shot script: set passwords for citiq_app and citiq_admin roles.

Run via Railway (uses the superuser DATABASE_URL injected by Railway):
    railway run --service citiq-api python scripts/setup_db_roles.py
...
async def main() -> None:
    ...
    try:
        ...
        await conn.execute(f"ALTER ROLE citiq_app   PASSWORD '{escaped_app}'")
        await conn.execute(f"ALTER ROLE citiq_admin PASSWORD '{escaped_admin}'")
        print("Passwords set.")

        # Verify
        rows = await conn.fetch(
            "SELECT rolname, rolcanlogin, rolbypassrls "
            "FROM pg_roles WHERE rolname IN ('citiq_app', 'citiq_admin') "
            "ORDER BY rolname"
        )
        for r in rows:
        ...
        # Connect as citiq_app to verify
        host_parsed = urllib.parse.urlparse(url)
        app_url = host_parsed._replace(
            netloc=f"citiq_app:{urllib.parse.quote(app_pw)}@{host_parsed.hostname}:{host_parsed.port or 5432}"
        ).geturl()
        app_conn = await asyncpg.connect(app_url)
        try:
            count = await app_conn.fetchval("SELECT COUNT(*) FROM runs")
            if count == 0:
                print("RLS smoke test PASSED — 0 rows visible to citiq_app without client_id set.")
            else:
        ...
        print(f"DATABASE_URL_ADMIN=postgresql+asyncpg://citiq_admin:{urllib.parse.quote(admin_pw)}@{host}:{port}/{dbname}")
        print(f"DATABASE_URL_APP=postgresql+asyncpg://citiq_app:{urllib.parse.quote(app_pw)}@{host}:{port}/{dbname}")
        print("----------------------------------------")
